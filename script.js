    ...
    function spinMealCube() {
        ...
        setTimeout(() => {
            ...
            spinBtn.disabled = false;
                    ...
                    // Reset radio buttons for new meal
        clearMealOptions();
        }, 2000);
    }
    ...
    document.querySelectorAll('input[name="mealOption"]').forEach(radio => {
        radio.checked = false;
    });
}
...
function lockMeal() {
    ...
    if (!selectedOption) {
        alert('Please select whether you want to eat out or home cook this meal!');
        return;
    }
    ...
    const mealToAdd = {
        ...
        mealOption: selectedOption.value, // Save the meal option
        locked: true
    };
        ...
        for (let i = 0; i < 42; i++) {
            ...
            if (mealHistory[dateString] && mealHistory[dateString].length > 0) {
                dayElement.classList.add('has-meals');
                    ...
                    // Create meal summary with options
        let mealSummary = '';
        if (mealHistory[dateString] && mealHistory[dateString].length > 0) {
            const meals = mealHistory[dateString];
            const eatOutCount = meals.filter(meal => meal.mealOption === 'eat-out').length;
            const homeCookCount = meals.filter(meal => meal.mealOption === 'home-cook').length;
            
            mealSummary = `${meals.length} meal${meals.length > 1 ? 's' : ''}`;
            if (eatOutCount > 0) mealSummary += `<br>🍽️ ${eatOutCount}`;
            if (homeCookCount > 0) mealSummary += `<br>🏠 ${homeCookCount}`;
        }
    ...
    function showDayDetails(date) {
        ...
        if (meals.length === 0) {
            ...
            <div class="meal-detail">
                ...
                <p><strong>Option:</strong> <span class="meal-option ${meal.mealOption}">${meal.mealOption === 'eat-out' ? '🍽️ Eat Out' : '🏠 Home Cook'}</span></p>
            </div>
        `).join('');
        }
